...
console.log("screen width : ",window.innerWidth);
    ...
    useEffect(()=>{
     ...
     const loginhandler = ()=>{
        ...
        console.log("req :",req);
        fetch(`http://localhost:9090/userdata/login`,{method:"POST",
    ...
    const signuphandler = ()=>{
     ...
     fetch("http://localhost:9090/userdata/signup",{method:"POST",
                                           headers:{"Content-Type":"application/json"},
                                           body:JSON.stringify(req)
                                          }).then(res=>res.json())
                                          .then(result=>{
    ...
    const responseFacebooksignup = (response) => {
     ...
     //socialmedia signup handler
          const socialmediasignuphandler = (userdata)=>{        
            fetch('http://localhost:9090/userdata/socialSignUp',{
              method:"POST",
              headers:{"Content-Type":"application/json"},
          ...
          const socialmedialoginhandler =(userdata)=>{
            fetch('http://localhost:9090/userdata/socialLogin',{
              method:"POST",
              headers:{"Content-Type":"application/json"},
              body:JSON.stringify(userdata)
             }).then(res=>res.json())
  ...
  <React.Fragment>
     <div className='logo-part'>
            <div className='row mx-auto'>
                ...
                <div className='col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2 mt-4 '> <center>
                ...
                <div className='col-12 col-sm-12 col-md-1 col-lg-4 col-xl-4'></div>
                <div className='col-12 col-sm-12 col-md-4 col-lg-2 col-xl-2 my-3 '>
                  {isloggedIn && (local !== undefined && local.length >0)?<span className='login' style={{color:"red"}}>{ local[0].FirstName || local[0].FullName} </span> : <button className="login" onClick={()=>{setLoginmodal(true)}}>Login</button>}
                </div>
                <div className='col-12 col-sm-12 col-md-4 col-lg-2 col-xl-2 my-3  '>
                   {isloggedIn && (local !== undefined && local.length >0)?<button className='signup ' onClick={()=>logouthandler()}>Logout</button>:<button className="signup " onClick={()=>{setCreateaccountmodal(true)}} >Create an Account</button>}
                </div>
                <div className='col-12 col-sm-12 col-md-1 col-lg-1 col-xl-1'></div>                
            </div>     
     </div>
  ...
  <Modal  isOpen={createaccountmodal}
             style={customStyles}
            >
               <span className='text-success' style={{fontSize:"20px"}} >create account</span>  <button className='btn btn-danger btn-close' onClick={()=>{setCreateaccountmodal(false)}} style={{float:'right'}}></button>
               <br/><br/>
              <form className='form'>
               ...
               <input type="text" className="form-control" placeholder="LastName" required onChange={(e)=>{setLastName(e.target.value);setSignupError(undefined)}} />
                <br/>
               <div className='text-center'>
                  <input type="button" value="register" className="btn btn-block btn-success" onClick={()=>{signuphandler()}} />&nbsp;
